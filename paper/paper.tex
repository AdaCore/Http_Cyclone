...
\section{Introduction}
    ...
    or HACL*~\cite{zinzindohoue2017hacl}. The weakness of the Everest project is that it needs to use the untrusted
    transport layer TCP. If a bug exists in the transport layer, it can jeopardize the higher level layer.
...
\section{Verification of the TCP protocol and the Socket API}
    ...
    This two parts have been replaced by SPARK code and the absence of run-time errors has been proved for
    these parts.
    However, to be integrated with the existing library, it has been necessary to reuse existing C code
    as an untrested black box, for which we cannot have strong guarantees. Thus, others techniques
...
\subsection{The SPARK programming language}
    ...
    grows constantly and rapidely, recently with the addition of pointers \cite{dross2020recursive}.
...
\end{figure}
    ...
    tend to call the API functions with wrong arguments and they tend to forget to forget to check the value of the return code of
    function call before continuing the processing. This can lead to incorrect program behaviors when an error happen in a function
    ...
    and the features of the SPARK language, in particular the functional specification.

\subsubsection{Using types to carry information}

    Strong-typing is a first mechanism to ensure correctness of program and exclude a range of programing errors. Meaningful names for
    ...
    both as unsigned integer and it returns a new socket in case of success.
    ...
    In SPARK, types can be constrained by a valid range of values. Another kind of constraints on types can appear in
...
\subsubsection{Order of the functions call}
    ...
    If a function $f_1$ must be executed before a function $f_2$, the strategy is to add a postconditions on the socket on $f_1$
    that is also a precondition for $f_2$. Then, for the precondition of $f_2$ to be true, the function $f_1$ must have be called first.
...
Pre => Is_Ip_Initialized(Sock.S_Remote_Ip_Addr);
    \end{lstlisting}
    and because the function \lstinline{Socket_Connect} is the only one that can initialize this value, the order can be respected
    only with valid code.
    ...
    tests. It helps to programmers to write correct programs.
...
\subsubsection{Checking error codes}
    ...
    \lstinline{(if Error = NO_ERROR then ...)} makes that \lstinline{S_Remote_Ip_Addr} is initialized only when no error happen during
    ...
    if it is known that no error happen previously.
...
\subsubsection{Rewritting TCP user functions}
    ...
    to wait for an actions to be executed by another thread. This action can be, for example, the reception of a segment. But when a
    ...
    procedure \lstinline{Tcp_Wait_For_Events} returns if we wants to know that there is no violation of the norm in the further transitions.
    ...
    \begin{itemize}
        ...
        \item Ces évènements sont donc à prendre en compte. Il faut savoir précisément quel est l'état de la socket quand la fonction retourne si on veut calculer de manière précise les états à la sortie de la fonction, et le fait qu'il n'y ait pas de violation de la norme lors des transitions.
