...
const PhyDriver ksz8795PhyDriver =
{
   ksz8795Init,
   ksz8795Tick,
   ksz8795EnableIrq,
...
{
   0,
   KSZ8795_TAIL_TAG_ENCODE(1),
   KSZ8795_TAIL_TAG_ENCODE(2),
   KSZ8795_TAIL_TAG_ENCODE(3),
   KSZ8795_TAIL_TAG_ENCODE(4)
};
            ...
            virtualInterface->parent == interface)
         {
            ...
            if(port >= KSZ8795_PORT1 && port <= KSZ8795_PORT4)
            {
               ...
               if(linkState && !virtualInterface->linkState)
               {
                  ...
                  status = ksz8795ReadSwitchReg(interface,
                     KSZ8795_PORTn_CTRL11_STAT3(port));

                  //Check current operation mode
                  switch(status & KSZ8795_PORTn_CTRL11_STAT3_OP_MODE)
                  {
                  //10BASE-T half-duplex
                  case KSZ8795_PORTn_CTRL11_STAT3_OP_MODE_10BT_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;
                  //10BASE-T full-duplex
                  case KSZ8795_PORTn_CTRL11_STAT3_OP_MODE_10BT_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;
                  //100BASE-TX half-duplex
                  case KSZ8795_PORTn_CTRL11_STAT3_OP_MODE_100BTX_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;
                  //100BASE-TX full-duplex
                  case KSZ8795_PORTn_CTRL11_STAT3_OP_MODE_100BTX_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;
   ...
   size_t *length, uint8_t *port)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
      {
         ...
         *port = KSZ8795_TAIL_TAG_DECODE(*tailTag);
