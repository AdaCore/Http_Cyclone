...
#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
      //Check whether port tagging is implemented
...
error_t ethSendFrame(NetInterface *interface, const MacAddr *destAddr,
   uint16_t type, NetBuffer *buffer, size_t offset,
   const NetAncillaryData *ancillary)
{
   error_t error;
   uint32_t crc;
   size_t length;
   ...
   {
      //Port separation mode disabled?
      if(port == 0)
      {
         //The application may force the destination port
         port = ancillary->port;
      }
