...
error_t udpProcessDatagram(NetInterface *interface, IpPseudoHeader *pseudoHeader,
   const NetBuffer *buffer, size_t offset, NetAncillaryData *ancillary)
{
   error_t error;
   uint_t i;
   size_t length;
   ...
   uint_t flags)
{
   ...
   {
      //Additional options can be passed to the stack along with the packet
      ancillary = NET_DEFAULT_ANCILLARY_DATA;
      ...
      ancillary.dontRoute = (flags & SOCKET_FLAG_DONT_ROUTE) ? TRUE : FALSE;
...
error_t udpSendDatagramEx(NetInterface *interface, const IpAddr *srcIpAddr,
   ...
   NetBuffer *buffer, size_t offset, const NetAncillaryData *ancillary)
{
   error_t error;
   size_t length;
   UdpHeader *header;
