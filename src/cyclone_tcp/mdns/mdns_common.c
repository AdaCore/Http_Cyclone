...
void mdnsProcessMessage(NetInterface *interface,
   ...
   const NetBuffer *buffer, size_t offset, const NetAncillaryData *ancillary,
   void *param)
{
   size_t length;
   DnsHeader *dnsHeader;
   ...
   do
   {
      ...
      if(destIpAddr != NULL)
      {
         ...
         error = udpSendDatagramEx(interface, NULL, MDNS_PORT, destIpAddr,
            destPort, message->buffer, message->offset, &ancillary);
         //Any error to report?
         if(error)
            break;
      }
      else
      {
         ...
         error = udpSendDatagramEx(interface, NULL, MDNS_PORT, &ipAddr,
            MDNS_PORT, message->buffer, message->offset, &ancillary);
         //Any error to report?
         if(error)
            break;
#endif
